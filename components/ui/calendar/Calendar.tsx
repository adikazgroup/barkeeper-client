...
const Calendar = forwardRef<HTMLInputElement, CalendarProps>(
  (
    ...
    // Refine the popup position once its real height is known. The measurement is
    // driven by a ResizeObserver (an external system) instead of an effect body, so
    // it also re-runs when the popup content changes size (e.g. month/year picker).
    const refinePosition = useCallback(() => {
      const popupEl = portalRef.current;
      if (!popupEl || !triggerRef.current) return;
    ...
    const setPortalNode = useCallback(
      (node: HTMLDivElement | null) => {
        portalRef.current = node;
        if (!node) return;

        const observer = new ResizeObserver(refinePosition);
                      ...
                      <div className="flex flex-1 gap-1">
                        <div className="flex-1 w-[80px]">
                          <SimpleSelect
                            ...
                            placeholder="Month"
                            className="h-8 border-border rounded-md bg-muted"
                          />
                        </div>
